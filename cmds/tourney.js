...
module.exports = (bot) => {
  ...
  async function resumeTourney() {
    try {
      const tournies = await client.tournaments.index({
      ...
      if (openT[0]) {
        ...
        tData.participants.forEach(
          (obj) => (tPlayers[obj.participant.misc] = obj.participant.id)
        );
        ...
        let earlyMatch = tData.matches
            .map((obj) => obj.match)
            .filter((match) => match.state === "open"),
          completeMatches = tData.matches
            .map((obj) => obj.match)
        ...
        console.log(
          `Found existing tournament: ${currentTourney} | Now resuming from Round ${tRound} with ${tCount} players.`,
          "OK"
        );
      }
    } catch (e) {
  ...
  function getTourneyData(id = currentTourney) {
    return client.tournaments.show(id, {
  ...
  async function makeTourney(msg, args = []) {
    ...
    if (msg && !helpers.hasModPerms(msg.member.roles)) {
      ...
      if (args.length) {
        ...
        setTimeout(() => {
          const cmd = Object.values(bot.commands).filter((cmd) =>
            cmd.label.includes("startcup")
          )[0];
    ...
    try {
      const cups = await client.tournaments.index({
        subdomain: `pocketbotcup`,
      });
      tNum =
        cups.filter((cup) => cup.url.includes(`${tType.toLowerCase()}cup`))
          .length + 1;
      ...
      // PB announces it
      bot.createMessage(tourneyChan, {
        content: `:trophy: A new ${
          args?.length ? name : `${tType} Cup`
    ...
    if (
      ...
      try {
        ...
        const cMatches = cTourney.matches
          ...
          .map((match) => {
            return `:regional_indicator_${match.identifier.toLowerCase()}: <@${
              cPlayers[match.player1_id]
            }> vs. <@${cPlayers[match.player2_id]}>`;
          });

        bot.createMessage(
          tChan,
          stripIndents`${
            !tid ? "Processed all players who checked in. " : ""
          }**Let the games begin! :mega:**

        Round 1 Matches :crossed_swords::
        ${cMatches.join("\n")}

        **Reminder**: Submit your scores in this channel with the format \`!score x-y\`, no more replays necessary! ${
          !tid ? "Matches are **best of 3**." : ""
        }`
        );
        ...
        bot.createMessage(
          tChan,
          "Couldn't start tournament, check console for errors."
        );
      }
    } else {
      try {
        ...
        bot.createMessage(
          tChan,
          "Not enough participants entered the tournament this week, so it will be cancelled. :frowning: Gather some more folks for next week!"
        );
        resetTourneyVars();
      } catch (err) {
        console.error(err);
        bot.createMessage(
          tChan,
          "Couldn't destroy tournament, check console for errors."
        );
      }
    }
  }
  ...
  async function finishTourney(tid) {
    ...
    try {
      const cTourney = await client.tournaments.finalize(tourney, {
        include_participants: 1,
      });
      ...
      bot.createMessage(
        tChan,
        stripIndents`The tournament has come to a close! :tada: Our winners are:
      ...
      bot.createMessage(
        x.memchan,
        stripIndents`${tName} has just finished, our winners are:
        ...
        ${
          notPBCup
            ? ""
            : "If you wanna participate, keep an eye out for the signups every Monday at 2/6PM EST!"
        }`
      );
  ...
  async function checkRound(msg) {
    ...
    try {
      ...
      const tURL = notPBCup
        ...
        : `http://pocketbotcup.challonge.com/${tType.toLowerCase()}cup_${tNum}`;
      const tChan = notPBCup ? x.tourney : tourneyChan;
      ...
      const bronze =
        roundMap[roundMap.length - 1] === 0 ? roundMap.pop() : false; // If we have a round 0 (bronze) at the end, pop it out
      ...
      const open = matches.filter(
        (match) => match.state === "open" && match.round == roundMap[tRound]
      );

      console.info(
        `Round ${roundMap[tRound]} - Open matches left: ${open.length} | ${roundMap}`
      );
      ...
      if (open.length === 0 && tRound < roundMap.length - 1) {
        ...
        console.log(
          `Starting Round ${tRound} (${roundMap[tRound]} in roundMap)`
        );

        let cMatches = t.matches
          .map((obj) => obj.match)
          .filter((match) => match.round === roundMap[tRound]) // Get the ones for current round
          .map((match) => {
            if (match.identifier !== "3P") {
              // For normal matches
              return `:regional_indicator_${match.identifier.toLowerCase()}: <@${
                cPlayers[match.player1_id]
              }> vs. <@${cPlayers[match.player2_id]}>`;
            } else {
              // For the bronze match
              return `:third_place: <@${cPlayers[match.player1_id]}> vs. <@${
                cPlayers[match.player2_id]
              }>`;
            }
          });

        if (
          (bronzeRound && tRound === roundMap.length - 3) ||
          (!bronzeRound && tRound === roundMap.length - 2)
        ) {
          bot.createMessage(
            tChan,
            stripIndents`**Semi-finals have begun! :mega:**
          ...
          bot.createMessage(
            tChan,
            stripIndents`**Our finalists are ready, but will now rest as we see who takes the bronze! :mega:**
          ${cMatches.join("\n")}
          ...
          bot.createMessage(
            tChan,
            stripIndents`**:trophy: :mega: And now for the grand finals! :mega: :trophy:**
          ${cMatches.join("\n")}
        ...
        msg.channel.createMessage(
          `Currently playing Round ${roundMap[tRound]} - Open matches left: ${open.length}`
        );
      }
    } catch (e) {
      console.error(e);
  ...
  async function addPlayer(msg, cid, tid = null, tRole = null) {
    ...
    if (tid && !cid) {
      msg.channel.createMessage(
        `<@${msg.author.id}>, you're going to need a Challonge account registered to join a custom tournament! Please use \`!challonge username\` to register your account, then you can remove/readd your :thumbsup: to try again.`
      );
      return false;
    }
    ...
    try {
      let player = await client.participants.create(cTourneyLocal, {
        participant: u,
      });
      ...
      if (tid) {
        ...
        const txt = `<@${msg.author.id}> has entered the tournament! ${
          cid ? ":comet:" : ""
        }`;
        if (msg.action) {
      ...
      if (tCount >= 17) {
        msg.channel.createMessage(
          `<@${msg.author.id}> is on standby! ${cid ? ":comet:" : ""}`
        );
      } else {
        const txt = `<@${msg.author.id}> has entered the Cup! ${
          cid ? ":comet:" : ""
        }`;
        if (msg.action) {
          msg.action.createMessage(txt);
      ...
      if (tCount === 16) {
        setTimeout(() => {
          msg.channel.createMessage(
            ":trophy: We have now reached the maximum amount of participants! I will announce check-ins later on."
          );
        }, 1000);
      }
    } catch (e) {
  ...
  async function deletePlayer(msg, cid, did, tid = null) {
    ...
    try {
      ...
      msg.channel.createMessage(
        `🕑 <@${did}> has been removed from the tournament. Hope you can play next time!`
      );
      if (tPlayers[cid]) delete tPlayers[cid];
    } catch (e) {
      msg.channel.createMessage(
        "🕑 Removal unsuccessful, guess you're stuck playing. Forever."
      );
  ...
  async function updateScore(msg, args) {
    ...
    if (!score) {
      msg.channel.createMessage(
        "What was the score? Use the syntax `!score x-y`, no spaces between the number and dash."
      );
      return false;
    }
    ...
    if (PBCup) {
      if (parseInt(score[0]) > 3 || parseInt(score[1]) > 3) {
        msg.channel.createMessage(
          "Those are some fishy scores. Double check they are correct!"
        );
        return false;
      }
    }
    ...
    if (score[0] > score[1]) {
      ...
      cTourney.matches.forEach((obj) => {
        ...
        if (obj.match.state === "open") {
          ...
          if (PBCup) {
            if (tPlayers[msg.author.id] == players[0]) {
              // Uploader is player 1, so winner is player2
              for (let w in tPlayers) {
                if (tPlayers[w] === players[1]) winnerID = tPlayers[w];
              }
              winnerCID = players[1];
            } else if (tPlayers[msg.author.id] == players[1]) {
              // Uploader is player 2, winner is player 1
              for (let w in tPlayers) {
                if (tPlayers[w] === players[0]) winnerID = tPlayers[w];
              }
              winnerCID = players[0];
            }
          } else {
            // Otherwise, do more hunting
            let myCID;

            // First get my challonge ID
            const allPlayers = cTourney.participants.map((p) => p.participant);
            allPlayers.forEach((p) => {
              if (p.misc == msg.author.id) myCID = p.id;
            });
    ...
    if (winnerCID === null || winnerCID === undefined) {
      msg.channel.createMessage(
        "Hmm, I couldn't find a proper Challonge ID to match your Discord ID. That's not good..."
      );
      return false;
    }
    ...
    matches.forEach((obj) => {
      console.log(
        `Analysing ${obj.id} | ${obj.round} for Round ${tRound}`,
        "Info"
      );

      if (obj.state === "open" && obj.round == roundMap[tRound]) {
        console.log(`Looking for ${winnerCID}...`);
        if (obj.player1_id == winnerCID || obj.player2_id == winnerCID)
          match = obj;
      }
    });
    ...
    try {
      ...
      if (score[0] > score[1] && match.player1_id == winnerCID) {
        await client.matches.update(currentTourney, match.id, {
          match: {
            scores_csv: `${score[0]}-${score[1]}`,
            winner_id: winnerCID,
          },
        });
      ...
      msg.channel.createMessage(
        "Score updated. :tada: Please wait for round to finish before reporting your next match."
      );
      msg.channel.createMessage(
        `<@${msg.author.id}> has updated their score. :tada:`
      ); // TODO - Replace with an embed replay summary?
      ...
      if (err.includes?.("Cannot read property 'id' of undefined")) {
        ...
        msg.channel.createMessage(
          `Dang it, something went wrong with the score submission. Try again? If you think you did everything correctly, talk to <@${x.stealth}>! :scream: \n \`\`\`Error: ${err}\`\`\``
        );
      }
    }
  }
    ...
    (msg, args) => {
      ...
      if (currentTourney) {
        return msg.channel.createMessage(
          "Cup currently in progress, try again later."
        );
      }
  ...
  bot.registerCommand(
    ...
    (msg) => {
      ...
      bot.createMessage(
        tourneyChan,
        `<@&${x.competitor}>s if you cannot play in the tournament starting soon, go ahead and \`!signout\`. Otherwise, good luck!`
      );
    },
    {
  ...
  bot.registerCommand(
    ...
    async (msg) => {
      ...
      if (!currentTourney) {
        return msg.channel.createMessage(
          "🕑 There are no Pocketbot Cups currently running."
        );
      }

      if (!tPlayers[msg.author.id]) {
        return msg.channel.createMessage(
          "🕑 You haven't even signed up, liar."
        );
      }
    ...
    (msg, args) => {
      ...
      if (process.env.LOCALTEST && !tPlayers.hasOwnProperty(msg.author.id)) {
        if (Object.values(tPlayers).length > 0) {
          msg.channel.createMessage(
            "🕑 Uhm...you don't seem to be in the tournament, so I'm going to go ahead and ignore you. :sweat_smile:"
          );
          return false;
        }
      }
  ...
  bot.registerCommand(
    ...
    async (msg, args) => {
      ...
      if (!currentTourney) {
        return msg.channel.createMessage(
          "🕑 There are no Pocketbot Cups currently running. :thinking:"
        );
      }
      ...
      if (!tPlayers.hasOwnProperty(player)) {
        return msg.channel.createMessage(
          "🕑 This user isn't even part of the tournament."
        );
      }
  ...
  bot.registerCommand("challonge", (msg) => {
    ...
    if (cidFB) {
      ...
      msg.channel.createMessage(
        "🕑 Huh. Something went wrong here. Talk to Mastastealth about it."
      );
    }
  });
  ...
  return {
    register() {
      console.info("Registered tournament commands.");
    },
  };
};
